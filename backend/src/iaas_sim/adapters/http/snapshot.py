...
from iaas_sim.adapters.http.virtual_machine import operation_resource
...
def create_snapshot_router(port: SnapshotPort, registry: OperationRegistryPort) -> APIRouter:
    router = APIRouter(prefix="/v1/snapshots", tags=["snapshots"])

    @router.get("")
    def list_resources() -> dict[str, list[dict[str, object]]]:
        match list_snapshots(port):
            case Err(error):
                _raise(error)
            case Ok(snapshots):
    ...
    def get_resource(snapshot_id: str) -> dict[str, object]:
        match get_snapshot(port, SnapshotId(snapshot_id)):
            case Err(error):
                _raise(error)
            case Ok(snapshot):
    ...
    def create_resource(body: CreateSnapshotInput) -> JSONResponse:
        result = create_snapshot(
            ...
            registry,
            OperationId(uuid7()),
            VirtualMachineId(body.virtualMachine.id),
            body.name,
        )
        match result:
            case Err(error):
    ...
    def delete_resource(snapshot_id: str) -> JSONResponse:
        result = delete_snapshot(port, registry, OperationId(uuid7()), SnapshotId(snapshot_id))
        match result:
            case Err(error):
                _raise(error)
            case Ok(operation):
