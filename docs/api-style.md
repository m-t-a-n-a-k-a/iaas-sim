...
The API exposes `POST /v1/virtualMachines` with exactly `name` and an embedded
...
only after the backend task succeeds and its VM identity is atomically finalized with the Operation's `SUCCEEDED` transition. The VM is created powered off; starting it remains an explicit command.
